    ...
    private static void expandItemTag(
        ...
        registry.getTag(key).ifPresent(set -> {
            for (Holder<Item> holder : set) {
                resolveHolder(registry, holder, visited, outItems, outTags, ClosureExpander::expandItemTag);
            }
        });
    }
    ...
    private static void expandBlockTag(
        ...
        registry.getTag(key).ifPresent(set -> {
            for (Holder<Block> holder : set) {
                resolveHolder(registry, holder, visited, outBlocks, outTags, ClosureExpander::expandBlockTag);
            }
        });
    }
    ...
    private static void expandFluidTag(
        ...
        registry.getTag(key).ifPresent(set -> {
            for (Holder<Fluid> holder : set) {
                resolveHolder(registry, holder, visited, outFluids, outTags, ClosureExpander::expandFluidTag);
            }
        });
    }
